    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("Uses Spotify Web API as the source of truth. Tokens are stored in Keychain. Add SPOTIFY_CLIENT_ID and redirect URI \(MinitapBrand.spotifyRedirectURI) to enable connection.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
...
struct About: View {
    ...
    var body: some View {
        VStack {
            ...
            VStack(spacing: 0) {
                Divider()
                Text("Built for modern mobile QA by minitap")
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
...
struct ClipboardSettings: View {
    ...
    @Default(.clipboardHistoryCaptureImages) var captureImages
    @Default(.clipboardHistoryShowSourceApps) var showSourceApps
    @Default(.clipboardHistoryPauseOnLock) var pauseOnLock
    @Default(.clipboardHistoryClearOnQuit) var clearOnQuit
    @Default(.clipboardHistoryMaxItems) var maxItems
    @Default(.clipboardHistoryMaxStorageMB) var maxStorageMB
    @ObservedObject private var clipboardHistory = ClipboardHistoryViewModel.shared
        ...
        Form {
            Section {
                ...
                .onChange(of: clipboardHistoryEnabled) {
                    if clipboardHistoryEnabled {
                        clipboardHistory.enableHistory()
                    } else {
                        clipboardHistory.disableHistory()
                    }
                }

                Defaults.Toggle(key: .clipboardHistoryCaptureImages) {
                    Text("Save copied images")
                }
                Defaults.Toggle(key: .clipboardHistoryShowSourceApps) {
                    Text("Show source app")
                }
                Defaults.Toggle(key: .clipboardHistoryPauseOnLock) {
                    Text("Pause while screen is locked")
                }
                Defaults.Toggle(key: .clipboardHistoryClearOnQuit) {
                    Text("Clear history when quitting")
                }
            } header: {
                Text("Capture")
            } footer: {
                Text("Clipboard history starts only after you enable it. Concealed, transient, and autogenerated pasteboard content is skipped.")
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
            ...
            Section {
                ...
                .onChange(of: maxItems) {
                    clipboardHistory.enforceRetentionLimits()
                }

                Stepper(value: $maxStorageMB, in: 25...1000, step: 25) {
                    HStack {
                        Text("Maximum image storage")
                        Spacer()
                        Text("\(maxStorageMB) MB")
                            .foregroundStyle(.secondary)
                    }
                }
                .onChange(of: maxStorageMB) {
                    clipboardHistory.enforceRetentionLimits()
                }
                ...
                HStack {
                    ...
                    Spacer()
                    Text(clipboardHistory.storageText)
                        .foregroundStyle(.secondary)
                }
            } header: {
                Text("Retention")
            } footer: {
                Text("Old unpinned items are removed first when either limit is reached.")
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
            ...
            Section {
                HStack {
                    VStack(alignment: .leading) {
                        Text("Saved items")
                        Text("\(clipboardHistory.items.count) total")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
        ...
        Form {
            Section {
                VStack(alignment: .leading, spacing: 16) {
                    // Toggle between brand and custom
                    Picker("Accent color", selection: $useCustomAccentColor) {
                        Text("Brand").tag(false)
                    ...
                    if !useCustomAccentColor {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            HStack(spacing: 12) {
                                AccentCircleButton(
                                    isSelected: true,
                                    color: MinitapBrand.Colors.accent,
                                    isSystemDefault: true
                                ) {}
                                
                                VStack(alignment: .leading, spacing: 2) {
                                    Text("Using minitap purple")
                                        .font(.body)
                                    Text("Brand default, #6A5AE0")
                                        .font(.caption)
                                        .foregroundStyle(.secondary)
                                }
                                Spacer()
                ...
                Text("Choose between the minitap brand accent or customize it with your own selection.")
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
...
struct AccentCircleButton: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .help(isSystemDefault ? "Use the minitap brand accent" : "")
    }
}
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("Opens the same clipboard history UI available from the menu bar icon.")
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
            Section {
                ...
                Stepper(value: $cursorScaleDuration, in: 0.5...30, step: 0.5) {
                    HStack {
                        Text("Timed duration")
                        Spacer()
                        Text("\(cursorScaleDuration, specifier: "%.1f")s")
                            .foregroundStyle(.secondary)
                    }
                }
                Slider(value: $cursorScaleAmount, in: 1.5...8, step: 0.5) {
                    HStack {
                        Text("Cursor size")
                        Spacer()
                        Text("\(cursorScaleAmount, specifier: "%.1f")x")
                            .foregroundStyle(.secondary)
                    }
                }
